...
SSRF_PROTECTION_ENABLED ?=false      # disable SRF protection to support private IP network.
AUTH_REQUIRED ?=false                # !!! may need to set true to make UAID_ALLOWED_DOMAINS takes empty.
...
start:
	...
	-e SSRF_PROTECTION_ENABLED=${SSRF_PROTECTION_ENABLED}           \
...
stop:
	-docker rm -f -v ${cName}
bash:
	docker exec -it -u root ${cName} /bin/bash
run:
	${cmd}
